...
function stopDrawing(e) {
    ...
    // Модуль 5: Зеркальная елочка - проверяем, была ли линия правильной
    if (currentExercise && currentExercise.type === 'mirror-tree') {
        if (!currentLineValid && userDrawnPoints.length > 5) {
            // Линия была неправильной - показываем ошибку и очищаем
            showMirrorTreeError('Не совсем так! Попробуй ещё раз');
            
            // Через 1.5 секунды перерисовываем шаблон (удаляем красную линию)
            setTimeout(() => {
                clearCanvas();
                drawMirrorTreeTemplate();
            }, 1500);
        }
        // Сбрасываем точки текущей линии
        userDrawnPoints = [];
        return;
    }
...
function drawMirrorTreeWithCheck(pos) {
    ...
    currentLineValid = false; // Сбрасываем флаг
    ...
    for (let i = 0; i < mirrorTreeTargets.length; i++) {
        ...
        if (distance <= treePathTolerance) {
            isOnSegment = true;
            currentLineValid = true; // Линия правильная!
    ...
    if (isOnSegment) {
        ...
        ctx.strokeStyle = '#ff5252'; // Красный - неправильно
        ctx.lineWidth = 4;
    }
